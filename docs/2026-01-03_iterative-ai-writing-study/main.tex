...
Contemporary large language models are trained on broad corpora and frequently exhibit what appears to be ``common-sense'' competence across many domains. This observation is widely discussed under the umbrella of ``foundation models'' \cite[Section~1]{Bommasani2021}, which emphasizes generality across tasks and domains. At the same time, there is substantial evidence that language models can produce fluent but false or misleading outputs \cite[Section~1]{Lin2021TruthfulQA}, motivating explicit evaluation of truthfulness and verification practices.
...
\begin{enumerate}
  ...
  \item Over iterations, the document may move toward a state in which a larger fraction of claims are grounded in cited sources or proved arguments. This outcome is aspirational and depends on reviewer expertise, the availability and quality of sources, and disciplined revision; failure modes (e.g., plausible but incorrect narratives) remain possible. Unresolved items are collected as open questions, potentially indicating directions for further study; see \cref{sec:open-questions}.
...
\end{tikzpicture}
\caption{A draft generation--review--verification loop. The model contributes text and candidate structure; the human retains responsibility for verification and for deciding which claims to keep. Each iteration should produce an explicit record of changes (e.g., resolved issues and remaining open questions) to support targeted subsequent review.}
\label{fig:loop}
\end{figure}
