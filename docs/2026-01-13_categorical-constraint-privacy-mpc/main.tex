...
\]
One concrete deterministic extraction from a channel $t$ is its \emph{support-valued} observation: define a deterministic map
\[
s : X_C\times X_{\bar C} \to \mathcal{P}(V_C)
\]
by sending each input $(x_C,x_{\bar C})$ to the support set of the induced law on $V_C$. Concretely,
\[
s(x_C,x_{\bar C}) \;:=\; \mathrm{Supp}(t(x_C,x_{\bar C})) \;=\; \{\,v\in V_C \mid \Pr[V_C=v\mid x_C,x_{\bar C}]>0\,\}.
\]
Treating $s$ as the observation, ``zero additional constraints'' corresponds to requiring that observing \emph{which} view-values are possible does not refine the equivalence relation induced by $h$, i.e.\ that the induced indistinguishability relation is unchanged:
\[
\sim_{h} \;=\; \sim_{(h,s)}.
\]
