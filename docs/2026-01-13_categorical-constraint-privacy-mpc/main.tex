...
\]
This note deliberately does not fix a particular threat model (semi-honest vs.\ malicious); the only assumption used below is that the notion of ``view'' has been chosen to match the intended adversarial power.
...
\]
If one extracts from $k$ a deterministic observation (e.g.\ by fixing randomness, or by taking support information), then ``zero additional constraints'' corresponds to requiring
\[
\sim_{h} \;=\; \sim_{(h,k)}.
\]
