...
\begin{abstract}
This note presents a diagrammatic framework for describing and comparing multi-party computation (MPC) protocols in a uniform language. Working in $\mathbf{FinSet}$, it fixes an execution object $E=(\prod_i X_i)\times\Omega$ and treats a protocol as a family of morphisms out of $E$. Correctness is expressed as a commuting square: the protocol-induced reconstruction $\rho\circ\pi_S:E\to Y$ agrees with the ideal functionality $f\circ\pi_X:E\to Y$. For security analysis, the base square is extended to an ambient diagram by adjoining observation morphisms $\pi_{\mathrm{full}}:E\to B_{\mathrm{full}}$ and their restrictions $\pi_A:E\to B_A$ for corruption patterns $A$. This ambient diagram serves as an \emph{arrow-translation template}: protocols are compared by instantiating the same pattern of objects and arrows and by analyzing how adversarial observations behave under post-processing. After choosing distributions (e.g.\ sampling protocol randomness and pushing forward along $\pi_A$), the template yields simplex-valued kernels and supports two complementary security layers: Layer~1 compares induced view laws (exactly or approximately, e.g.\ via total variation or KL), while Layer~2 studies posterior structure within feasible fibers determined by inevitable constraints (typically the output). The method is illustrated on Shamir secret sharing \cite{Shamir1979} and on representative protocol motifs, emphasizing how corruption growth and protocol post-processing correspond to monotone information refinement (data processing) \cite{CoverThomas2006}.
\end{abstract}
...
This document is maintained as research notes. Citations have been added only for a small set of foundational touchpoints (e.g.\ secret sharing \cite{Shamir1979}, data processing and information measures \cite{CoverThomas2006}, and information-geometric background \cite{AmariNagaoka2000}); further pinpoint citations should be added where specific external claims are relied upon.
